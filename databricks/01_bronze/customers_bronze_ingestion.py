...
try:
    log_message("INFO", "Starting Bronze Ingestion for Customers (CSV)")
    ...
    null_id_count = df_bronze.filter(col("customer_id").isNull()).count()

    if null_id_count > 0:
        log_message("WARNING", f"Found {null_id_count} records with NULL customer_id")

    duplicate_count = df_bronze.groupBy("customer_id").count().filter(col("count") > 1).count()

    if duplicate_count > 0:
        log_message("WARNING", f"Found {duplicate_count} duplicate customer_id values in batch")
    ...
    log_message("INFO", "Customers successfully written to Bronze table")
